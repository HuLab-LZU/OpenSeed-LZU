#!/bin/bash
# Build (if needed) and benchmark mnn_bench on Linux.
#
...
# DATASET_JSON="${DATASET_JSON:-data/bench_dataset/cls_656_30_rgb_st_crop_test.json}"
...
for MODEL in "${MODELS[@]}"; do
    ...
    RUN_ARGS=(--model "$MODEL" --backend "$BACKEND" --threads "$THREADS" \
              --warmup "$WARMUP" --repeat "$REPEAT" --save-pred)
